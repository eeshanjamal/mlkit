...
class Camera2Source(private val graphicOverlay: GraphicOverlay) {
    ...
    /** Background thread (including handler) for camera frame processing.  */
    ...
    internal fun start(textureView: TextureView) {
        if (camera != null) return

        camera = createCamera(textureView).apply {
     ...
     */
    @Throws(IOException::class)
    private fun createCamera(textureView: TextureView): CameraDevice {
        ...
        for (cameraId in cameraManager.cameraIdList) {
            ...
            if (cameraCharacteristics.get<Int>(CameraCharacteristics.LENS_FACING) == CAMERA_FACING_BACK) {
                val streamConfigurationMap = cameraCharacteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP)
                val sizes = streamConfigurationMap!!.getOutputSizes(SurfaceTexture::class.java)
                previewSize = getPreviewSize(sizes)
        ...
        selectedCameraId?.let {
            ...
            if (ActivityCompat.checkSelfPermission(context, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                ...
                backgroundHandlerThread = HandlerThread("camera_background_thread").apply {
                    start()
                ...
                cameraManager.openCamera(it, object: CameraDevice.StateCallback() {
                    override fun onOpened(camera: CameraDevice) {
                        this@Camera2Source.camera = camera
                        createPreviewSession(textureView, previewSize!!, camera, backgroundHandler!!)
                    }

                    override fun onDisconnected(camera: CameraDevice) {
                        camera.close()
                        this@Camera2Source.camera = null
                    }
                ...
                throw IOException("Camera access permission not granted")
            }
    ...
    private fun createPreviewSession(textureView: TextureView, previewSize: Size, cameraDevice: CameraDevice, backgroundHandler: Handler) {
        ...
        captureRequestBuilder = cameraDevice.createCaptureRequest(CameraDevice.TEMPLATE_PREVIEW).apply {
            ...
            cameraDevice.createCaptureSession(listOf<Surface>(previewSurface), object : CameraCaptureSession.StateCallback() {
    ...
    @Throws(IOException::class)
    private fun getPreviewSize(size: Array<out Size>): Size{
        // Gives priority to the preview size specified by the user if exists.
        val previewSize = PreferenceUtils.getUserSpecifiedPreviewSize(context)?.preview?.let { Size(it.width, it.height) } ?: run {
            ...
            selectSize(size, displayAspectRatioInLandscape)
        } ?: throw IOException("Could not find suitable preview size.")

        return previewSize.also {
     ...
     */
    private fun getCameraRotationAngle(camera: CameraDevice, characteristics: CameraCharacteristics): Int {
    ...
    companion object {
         ...
         */
        private fun selectSize(previewSizes: Array<out Size>, displayAspectRatioInLandscape: Float): Size? {
            ...
            for (previewSize in previewSizes) {
                ...
                if (abs(aspectRatioDiff - minAspectRatioDiff) < Utils.ASPECT_RATIO_TOLERANCE) {
                    if (selectedSize == null || selectedSize.width < previewSize.width) {
            ...
            if (selectedSize == null) {
                ...
                for (previewSize in previewSizes) {
                    val diff =
                        abs(previewSize.width - DEFAULT_REQUESTED_CAMERA_PREVIEW_WIDTH) +
                                abs(previewSize.height - DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT)
                    if (diff < minDiff) {
                        selectedSize = previewSize
                        minDiff = diff
                    }
